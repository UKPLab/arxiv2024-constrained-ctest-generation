...
def get_difficulty(feedback):
    difficulty = {"too hard":5,
                  "hard":4,
                  "OK":3,
                  "easy":2,
                  "too easy":1}
    return difficulty[feedback]
...
with open('r_data/data_feedback.csv','w', newline='', encoding='utf-8') as f:
    writer = csv.writer(f,delimiter=',')
    tableheader = ["index","feedback","time","user_key","user_idx","model","tau","text","cefr","years","frequency"]
    ...
    for user_idx, user_vals in user_data.items():
        ...
        for ctest_idx, ctest_vals in sorted(ctest_user_data.items()):
            ...
            writer.writerow([idx, ctest_vals['feedback'], get_time_in_seconds_max(ctest_vals['time'], max_time), user_vals['key'], user_idx, model, diff, text, user_questionnaire['cefr'], user_questionnaire['years'], user_questionnaire['frequency']])
            idx += 1
