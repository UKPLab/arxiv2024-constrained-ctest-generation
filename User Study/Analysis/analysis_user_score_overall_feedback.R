...
#############################
# data loading
data = read.csv('r_data/data_feedback.csv')
...
difficulty_model <- bam(feedback ~ model + tau + s(text, bs="re") + s(cefr, bs="re") + s(user_key, bs="re") + s(years) + model:tau, #
                        data=data,
                        method="fREML",
                        family=ocat(R=5),
                        discrete=TRUE)
...
saveRDS(difficulty_model, "r_models/difficulty_feedback.rds")
dependencies = c("mgcv")
file_obj <- file("r_models/difficulty_feedback.dep")
