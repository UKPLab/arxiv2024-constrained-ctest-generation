...
def main():
    ...
    opt_model.addConstr(gp.quicksum(position_decision_variables) == 20)
    ...
    opt_model.addConstr(total_gap_sum == gp.quicksum(capped_outputs)/20)
    ...
    with open(os.path.join(args.output_folder,f"{args.output_name}_{args.model.split('/')[-1]}_{infile_name}_{tau}"),'w') as f:
        json.dump(results, f)       
