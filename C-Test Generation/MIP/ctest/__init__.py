...
class GapManipulator:
    def __init__(self, dictionary_path: str, update_bert: bool=False) -> None:
        # NEW INDEXING:
        # IDX 49: BERT pred prop -> OPTIONAL, DONE
        # IDX 50: BERT top50 entropy -> OPTIONAL, DONE
        # IDX 56: CompoundBreak -> 0/1  -> DONE  -> NOTE: Only do this, if the feature IsCompound is true!
        # IDX 57: IsReferentialGap (TH-gap) -> Done 
        # IDX 58: SyllableBreak -> 0/1  -> DONE
        # IDX 59: LengthOfSolutionChars -> DONE  -> NOTE: This is the only thing we can manipulate.
        # IDX 60: LengthOfSolutionSyllables -> DONE
...
class PositionManipulator:
    def __init__(self) -> None:
        # Requires the loaded data in the form of a dict:
        # keys are pairs: (gap_idx, sent_idx)
        # values are: words
        # IDX 51: NumberOfGapsInSentence -> Number of Gaps in Sentence 
        # IDX 52: NumberOfPrecedingGaps -> Gap IDX
        # IDX 53: NumberOfPrecedingGapsInCoverSentence -> Number of prev Gaps in Sentence
        # IDX 54: OccursAsGap -> 0/1 Same gap exists
        # IDX 55: PositionOfGap -> Token IDX
    ...
    def get_pos_idx(self, data: dict, gap_idx: list) -> dict:
        ...
        for idxs, val in self.c_test.items():
            ...
            #new_features[self.feature_index_dict['gap_token_index']] = self.get_position_of_gap(gap_idx)
            result[idxs] = new_features
    ...
    def get_pos_features(self, data: dict, gap_idx: list) -> dict:
        ...
        for idxs, val in self.c_test.items():
            ...
            #feature[self.feature_index_dict['gap_token_index']] = self.get_position_of_gap(gap_idx)
