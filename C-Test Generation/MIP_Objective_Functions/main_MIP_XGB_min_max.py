...
def main():
    ...
    # Use max(tau, tau-hat) - min(tau, tau-hat)
    ################################
    
    final_min_output = opt_model.addVar(vtype=gp.GRB.CONTINUOUS)
    final_max_output = opt_model.addVar(vtype=gp.GRB.CONTINUOUS)
    
    opt_model.addGenConstrMax(final_max_output, [total_gap_sum], tau)
    # Compute max and min for the set {output, tau} 
    opt_model.addGenConstrMin(final_min_output, [total_gap_sum], tau)
        
    logging.info("====> Running optimization for positions (with variable gap size)")

    opt_model.setObjective(final_max_output - final_min_output, gp.GRB.MINIMIZE)
    ...
    with open(os.path.join(args.output_folder,f"{args.output_name}-minmax_{args.model.split('/')[-1]}_{infile_name}_{tau}"),'w') as f:
        json.dump(results, f)       
