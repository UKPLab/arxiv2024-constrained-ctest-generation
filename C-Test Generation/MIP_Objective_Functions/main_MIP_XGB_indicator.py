...
def main():
    ...
    # Select **exactly** args.num_gaps gaps
    ...
    # Final difficulty is the sum over all gaps divided by the number of gaps (args.num_gaps)
    ...
    ################################
    output_selector = opt_model.addVars(2, vtype=gp.GRB.BINARY, name="output_selector")
    # If output < tau; then tau - output:
    opt_model.addGenConstrIndicator(output_selector[0], True, total_gap_sum, gp.GRB.LESS_EQUAL, tau)
    # If tau < output; then output - tau:
    opt_model.addGenConstrIndicator(output_selector[1], True, total_gap_sum, gp.GRB.GREATER_EQUAL, tau)
    # We minimize the sum of each case
    opt_model.setObjective(gp.quicksum([output_selector[0]*(tau-total_gap_sum), output_selector[1]*(total_gap_sum-tau) ]), gp.GRB.MINIMIZE)
    ...
    with open(os.path.join(args.output_folder,f"{args.output_name}-indicator_{args.model.split('/')[-1]}_{infile_name}_{tau}"),'w') as f:
        json.dump(results, f)       
